...
def scrape_food_news():
    ...
    for url in news_list:
        try:
            ...
            # 1. Try specific containers
            container_classes = ["col-md-12", "pad-top-0", "col-span-12 lg:col-span-9"]
            ...
            # 3. CRITICAL: Capture the result and append to the list
            if header_tag:
                news_title = header_tag.text.strip()
                all_headlines.append({"url": url, "title": news_title})
                print(f"Success: {news_title}")
            else:
                print(f"Failed to find title for: {url}")

        except Exception as e:
            print(f"Error scraping {url}: {e}")

    return all_headlines # This will now contain the data for FastAPI to send
