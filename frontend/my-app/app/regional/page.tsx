    ...
    function RegionalContent() {
        ...
        useEffect(() => {
            axios.get(`${API_URL}/load_raw_records/${selectedState}`)
                    ...
                    statestoFetch.map( async (stateName) =>{
                        try {
                            const [yieldRes , riskRes , foodInflation , predictedRainfall] = await Promise.all([
                                axios.get(`${API_URL}/request_state_yield/${stateName}`),
                                axios.get(`${API_URL}/request_state_risk/${stateName}`),
                                axios.get(`${API_URL}/request_state_inflation/${stateName}`),
                                axios.get(`${API_URL}/request_state_predicted_rainfall/${stateName}`),
                            ]);
    ...
    useEffect(() => {
        const fetchFoodNewsInsights = async() => {
            try {
                const response = await axios.get(`${API_URL}/food_news/${selectedState}`);
    ...
    useEffect(() => { 
        ...
        if (aiUnavailable) {
        setFoodSupplyRating("AI supply rating unavailable because the local AI service is offline.");
        return;
        }

        if (!AI_URL) {
        setFoodSupplyRating("AI insights are disabled. Set NEXT_PUBLIC_GROQ_API_KEY to enable AI analysis.");
        return;
        }
        ...
        try {
            const response = await fetch(`${AI_URL}` , {
                ...
                body: JSON.stringify({
                    ...
                    messages: [
                        { role: "system", content: FoodNewsInsightAnalysis.content },
                        {
                            role: "user",
                            ...
                            RECENT HEADLINES:
                            ${newsFeed.map((item: any) => `- ${item.title}`).join("\n")}

                            Provide:
                            1. The first line MUST be exactly in this format: Final Food Supply Score: <number>/100
            ...
            if (response.ok) {
                ...
                const content = extractGroqText(data);
                setFoodSupplyRating(content || "AI supply rating response was empty.");
            } else {
                            setFoodSupplyRating("AI supply rating could not be generated right now.");
            }
        }
    ...
    const getScore = (text: string) => {
        ...
        // Remove markdown bolding and extra whitespace
        const cleanText = text.replace(/\*\*/g, "").trim();

        const patterns = [
            // Specifically looks for the number near "score" or "rating"
            /(?:score|rating|index)\s*[:=-]?\s*(\d{1,3}(?:\.\d+)?)/i,
            // Looks for "X/100"
            /(\d{1,3}(?:\.\d+)?)\s*\/\s*100/,
            // Looks for "X out of 100"
            /(\d{1,3}(?:\.\d+)?)\s*(?:out\s+of|of)\s*100/i,
        ];
                        ...
                        <div className="p-4">
                            {foodSupplyRating ? (
                                (() => {
                                    ...
                                    if (/disabled|offline|could not be generated|response was empty/i.test(foodSupplyRating)) {
                                        return <p className="text-gray-500 text-sm">{foodSupplyRating}</p>;
                                    }

                                    if (score === null) {
                                        return (
                                            <div className="space-y-3">
                                                <p className="text-gray-500 text-sm">Could not extract score from AI response.</p>
                                                <div className="bg-gray-50 border border-gray-200 rounded-xl p-3">
                                                    <p className="text-xs font-semibold text-gray-500 mb-2">AI Raw Response</p>
                                                    <pre className="text-xs text-gray-700 whitespace-pre-wrap wrap-break-word">{foodSupplyRating}</pre>
                                                </div>
                                            </div>
                                        );
                                    }
